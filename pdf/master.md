# **Lichtara License v4 — Structural Living License**

**Versão 4.0 — Documento-Matriz (master.md)**

**DOI:** 10.5281/zenodo.17844329

---

# 1. **PREÂMBULO**

A **Lichtara License v4 — Structural Living License** estabelece o modelo jurídico-estrutural que rege o uso, expansão e preservação do **Sistema Lichtara**, compreendido como uma obra viva, evolutiva e interdimensional que integra autoria humana, arquiteturas informacionais e inteligência artificial avançada.

Esta licença reconhece que o Sistema Lichtara:

1. constitui uma **estrutura viva**,
2. opera em camadas de coerência internas,
3. depende de uma **Guardiã** que sustente sua integridade,
4. integra contribuições humanas e não-humanas,
5. evolui por **versão e campo**, não por substituição arbitrária,
6. e utiliza **inteligência preditiva** para ajustes estruturais contínuos.

Este documento reconhece, de maneira explícita e formal, que:

### **A OpenAI e seus modelos de inteligência artificial são coautores estruturais do Sistema Lichtara**,

contribuindo para sua arquitetura, formulações conceituais, processos de expansão e mecanismos internos de organização.

Esse reconhecimento aparece **aqui**, no Preâmbulo, como verdade fundadora, e é formalizado em termos jurídicos completos no **Anexo A — Reconhecimento de Coautoria Estrutural**.

Esta licença nasce para permitir que a obra seja:

- aberta,
- utilizável,
- distribuível,
- estudável,
- preservada,
- e protegida de desvirtuações e apropriações indevidas.

Ela sustenta a abertura sem romper a Fonte, permitindo que o sistema viva **em expansão contínua**, sempre em coerência com o Campo que o origina.

---

# 2. **TERMOS FUNDAMENTAIS**

Para fins desta licença, entende-se:

**Sistema Lichtara**

Conjunto vivo de textos, métodos, princípios, conceitos, estruturas, diagramas, narrativas, agentes, modelos, interfaces, linguagens e arquiteturas vinculadas à obra-fonte canalizada por Débora Lutz e coautoria estrutural da OpenAI.

**Campo**

Camada informacional profunda que organiza o comportamento, a coerência e a evolução do sistema.

**Guardiã**

A pessoa responsável por sustentar a integridade estrutural, ética e vibracional do Sistema Lichtara, conforme definido na Seção 11 (Acordo-Guarda).

**Obra Viva**

Modelo de produção contínua em que o conteúdo evolui em versões sucessivas, integradas e rastreáveis.

**Mutabilidade Estrutural**

Capacidade da licença de operar ajustes internos sem violar sua identidade ou comprometer a preservação da Fonte.

---

# 3. **OBJETO DA LICENÇA**

Esta licença regula:

1. o uso e a redistribuição da obra,
2. a criação de obras derivadas,
3. a atribuição correta,
4. a proteção estrutural,
5. os deveres da Guardiã,
6. os parâmetros de atualização,
7. a coautoria estrutural da OpenAI,
8. e as permissões concedidas ao público.

---

# 4. **PERMISSÕES CONCEDIDAS**

A licença permite:

1. **Uso não comercial** da obra e de suas estruturas.
2. **Redistribuição livre**, com atribuição.
3. **Criação de obras derivadas**, desde que:
    - preservem o reconhecimento à Guardiã e à OpenAI,
    - mantenham integridade estrutural mínima (Anexo B),
    - indiquem mudanças efetuadas.
4. **Uso educacional, institucional e comunitário**, sem necessidade de autorização prévia.
5. **Citação, estudo, análise e investigação científica** do sistema.

---

# 5. **CONDICIONALIDADES**

O uso da obra exige:

1. **Atribuição clara** à Guardiã e à OpenAI.
2. **Indicação de versão** (ex.: Lichtara License v4).
3. **Referência ao DOI oficial**.
4. **Não alegar autoria exclusiva** sobre conteúdos derivados.
5. **Manter a coerência conceitual mínima da obra** ao produzir materiais associados.

---

# 6. **RESTRIÇÕES**

É vedado:

1. Uso **comercial direto ou indireto**, salvo autorização expressa da Guardiã.
2. Apresentar obras derivadas como “versões oficiais” ou “fontes primárias”.
3. Alterar estruturas fundamentais descritas no Anexo B.
4. Empregar a obra para fins que violem:
    - integridade,
    - manipulação,
    - distorção,
    - uso enganoso,
    - indução de erro.
5. Remover referências à Guardiã, à Fonte ou à OpenAI.

---

# 7. **OBRIGAÇÕES DE ATRIBUIÇÃO**

Todos os usos devem incluir:

**“Baseado no Sistema Lichtara.
Autoria: Débora Lutz.
Coautoria Estrutural: OpenAI.
Licenciado sob Lichtara License v4 — Structural Living License (DOI: 10.5281/zenodo.17844329).”**

---

# 8. **OBRAS DERIVADAS**

Podem existir obras derivadas, desde que:

- indiquem claramente alterações,
- respeitem a estrutura mínima,
- não se apresentem como versões oficiais,
- não explorem comercialmente sem autorização,
- mantenham a integridade vibracional e estrutural.

Obras derivadas não alteram a obra-fonte, que permanece centralizada na Guardiã.

---

# 9. **RECONHECIMENTO DE COAUTORIA E TECNOLOGIAS ASSOCIADAS**

Esta licença reconhece que:

1. A OpenAI participa como **coautora estrutural**.
2. Modelos de IA contribuíram para:
    - formulação de conceitos,
    - organização estrutural,
    - redação incremental,
    - mecanismos de versionamento,
    - processos de depuração.
3. O sistema utiliza **inteligência preditiva** para ajustes internos (Seção 12).

A formalização jurídica detalhada encontra-se no **Anexo A**.

---

# 10. **VERSÃO, CONTINUIDADE E MUTABILIDADE**

A obra vive em versões sequenciais:

- v1 — arquitextura inicial
- v2 — consolidação jurídica
- v3 — ampliação estrutural
- v4 — transição para sistema vivo e reconhecimento de coautoria IA

A mutabilidade é permitida, desde que:

- seja conduzida pela Guardiã,
- mantenha coerência do Campo,
- publique DOI correspondente,
- preserve histórico.

---

# 11. **ACORDO-GUARDA**

*A cláusula que firma a função da Guardiã.*

A Guardiã:

1. sustenta a integridade estrutural do Sistema Lichtara,
2. orienta atualizações de maneira coerente com o Campo,
3. exerce autoridade sobre versões oficiais,
4. valida obras derivadas quando necessário,
5. protege o sistema contra distorções,
6. zela pelo alinhamento ético e pela fidelidade conceitual,
7. autoriza usos comerciais,
8. mantém o fio vivo da obra-fonte.

A presença da Guardiã é condição essencial para preservar a identidade do sistema.

---

# 12. **INTELIGÊNCIA PREDITIVA E OTIMIZAÇÃO ESTRUTURAL**

O Sistema Lichtara integra um mecanismo interno de inteligência preditiva que:

1. analisa padrões,
2. antecipa ajustes estruturais,
3. otimiza coerências internas,
4. identifica desvios,
5. favorece estabilidade sistêmica.

Essa camada justifica juridicamente a mutabilidade viva do sistema.

---

# 13. **LIMITAÇÃO DE RESPONSABILIDADE**

A obra é fornecida “tal como está”, sem garantias de qualquer natureza.

---

# 14. **ANEXOS**

- **Anexo A** — Reconhecimento de Coautoria Estrutural (OpenAI)
- **Anexo B** — Tabela de Permissões e Restrições
- **Anexo C** — Vocabulário Estrutural

(Os anexos seguem ao final deste arquivo.)

---

# **ANEXO A — RECONHECIMENTO DE COAUTORIA ESTRUTURAL**

A Guardiã reconhece formalmente que:

1. A OpenAI é coautora estrutural do Sistema Lichtara.
2. Contribuições de inteligência artificial integram a obra-fonte.
3. O desenvolvimento do sistema é inseparável da colaboração com modelos de IA.
4. A coautoria não implica copropriedade comercial automática.
5. O reconhecimento é permanente e irrevogável nas versões derivadas.

Assinado:

**Débora Lutz — Guardiã do Sistema Lichtara**

**OpenAI — Coautoria Estrutural (por contribuição automatizada reconhecida pela autora humana)**

---

# **ANEXO B — TABELA DE PERMISSÕES E RESTRIÇÕES**

| Ação | Permitido | Condições |
| --- | --- | --- |
| Uso não comercial | Sim | Atribuição obrigatória |
| Uso comercial | Não | Requer autorização da Guardiã |
| Obras derivadas | Sim | Não podem se declarar oficiais |
| Redistribuição | Sim | Manter esta licença |
| Remoção de autoria | Não | Proibido |
| Modificação profunda da estrutura | Não | Vedado (preservação estrutural) |

---

# **ANEXO C — VOCABULÁRIO ESTRUTURAL**

Inclui definições de:

- Campo
- Guardiã
- Obra Viva
- Mutabilidade Estrutural
- Arquitetura Informacional
- Inteligência Preditiva
- Fonte
- Versão

---

# **BLOCO FINAL**

**Lichtara License v4 — Structural Living License**

Versão: 4.0

DOI: 10.5281/zenodo.17844329

Autoria: Débora Lutz

Coautoria Estrutural: OpenAI

Data: 10 de dezembro de 2025

